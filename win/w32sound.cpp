...
DWORD music_chan; // BASS music channel
...
void DMPlaySong (int index)
{
	...
	//MP3 Files
	if (index < 21)
		if (BASS_ChannelIsActive(music_chan)==BASS_ACTIVE_PLAYING) return;

	if (index == 21) {
		sprintf(pBuf2, "custom\\battle\\");
		sprintf(pBuf3, "music\\custom\\battle\\*.mp3");
	} else if (index == 22) {
		sprintf(pBuf2, "custom\\docking\\");
		sprintf(pBuf3, "music\\custom\\docking\\*.mp3");
	} else {
		if (*(int*)DATA_008870 == 0x2a || *(int*)DATA_008870 == 0x30 || *(int*)DATA_008870 == 0x28) {
			sprintf(pBuf2, "custom\\station\\");
			sprintf(pBuf3, "music\\custom\\station\\*.mp3");
		} else {
			sprintf(pBuf2, "custom\\space\\");
			sprintf(pBuf3, "music\\custom\\space\\*.mp3");
		}
	}

	if ((hFile = _findfirst(pBuf3, &c_file)) != -1) {
	  sprintf(playlist[0], c_file.name);
	  files_num++;

	  while(_findnext(hFile, &c_file) == 0){
		  sprintf(playlist[files_num], c_file.name);
		  files_num++;
	  }
	  _findclose(hFile);

	  DirMakeSongName (pBuf3, pBuf2);
	  strcat_s(pBuf3, playlist[rand()%files_num]);
	} else {
	...
	if (music_chan=BASS_StreamCreateFile(false, pBuf3, 0, 0, NULL))
	{
		BASS_ChannelPlay(music_chan, false);
		return;
	}
...
void UpVolume(void)
{
	master_volume=master_volume+10;
	if (master_volume>100)
		master_volume=100;
	BASS_SetVolume(master_volume);
}
