	...
	if(fd)
	{
		...
		while(fgets(buf,512,fd))
		{
			...
			if(*t=='[')
			{
				...
				cfg->currentsection->name=_strdup(t);
			...
			else if(*t=='=')
			{
				...
				*(p+1)=0;
				...
				cfg->currentsection->keys[cfg->currentsection->keycount-1].data=_strdup(t);

				t=buf+strspn(buf," ");
				if((p=strchr(t,' '))) *p=0;

				cfg->currentsection->keys[cfg->currentsection->keycount-1].name=_strdup(t);
			}
		}
